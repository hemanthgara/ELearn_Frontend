...
const Comment = ({ id = "" }) => {
  ...
  const { comments, replies, fetchingComment } = useSelector((state) => state.commentReducer);
              ...
              return <div key={index}>
                <div className="comment my-2 fs-5 position-relative">
                  <div className="py-2 d-flex justify-content-between ">
                    ...
                    {pathname === "/viewqueries" ? <Link to={`/videodetail/${comment.video_id}`}>See video</Link> : <span></span>}
                  </div>
                  <h6 className="fw-bold fs-4">{comment.title}</h6>
                  <div className="d-flex justify-content-between align-items-end">
                    <p className="" style={{ width: "85%" }}>{comment.message}</p>
                    <div className="text-end text-primary fs-6" style={{ width: "15%" }}>
                      <FaUserEdit /> <span className="fs-6">{comment?.user_name}</span>
                    </div>
                  </div>
                  <FaReplyAll role={'button'} className="position-absolute" style={{ bottom: 0, right: '10px' }} onClick={() => setReplyCommentId([comment._id])} />
                </div>

                {replyCommentId.includes(comment._id) && <CommentReplyForm comment={comment} setReplyCommentId={setReplyCommentId} />}

                {replies.length > 0 && replies.map((reply) => {
                  if (reply.comment_id === comment._id) {
                    return <div key={reply._id} className="comment my-2 fs-5" style={{ width: '90%', marginLeft: 'auto' }}>
                      <div className=" d-flex justify-content-between ">
                        <p className="" style={{ width: "85%" }}>{reply.message}</p>

                        <div className="">
                          {pathname === "/viewqueries" ? <Link to={`/videodetail/${reply.video_id}`}>See video</Link> : <span></span>}

                          <div className="text-end text-primary py-3 fs-6">
                            <FaUserEdit /> <span className="fs-6">{reply?.user_name}</span>
                          </div>
                        </div>
